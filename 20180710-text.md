...
`Detected: False` と表示されているラベルの色を変更しましょう。編集する箇所はサンプルコードの`camera/processor/qr_detector.py`の38行目です。6つめの引数が`(Blue, Green, Red)`の順番で0 ~ 255の範囲で指定されます（OpenCVでは、一般的にRGBではなくBGRで色の指定をおこないます）。
...
同じファイル`camera/processor/qr_detector.py`に認識したQRコードの位置を描画するメソッドが実装されています。その呼び出しを行っている箇所のコメントを外してコミットを作成し、アプリケーションを更新しましょう。
